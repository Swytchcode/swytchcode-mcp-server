...
async function apiCall<T>(url: string, body: any): Promise<T | null> {
    const headers = {
...
server.tool(
  ...
  "Get code for a given method in a given language and library version",
  {
  ...
  async ({ service, version, language, method }) => {
    ...
    const generateCodeData = await apiCall<APIOutInterface>(generateCodeUrl, {
    ...
    const decodedCode: string = Buffer.from(code.code, 'base64').toString('utf-8');
...
server.tool(
  ...
  "List all the possible methods, workflows, services and versions",
    {
      service: z.string().describe("Name of the service like Stripe, Paypal, etc."),
      version: z.string().describe("Version of the service like v1, v2, etc."),
    ...
    async ({ service, version, param, regex_input }) => {
      ...
      const generateListData = await apiCall<APIOutInterface>(generateListUrl, {
      ...
      const concatenatedNames = generateListData.data.map((item: { name: any; }) => item.name).join('\n');
